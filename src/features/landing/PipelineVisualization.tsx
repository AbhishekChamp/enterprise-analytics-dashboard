...
export function PipelineVisualization() {
  ...
  const nodes: PipelineNode[] = [
    { id: 'ingest', x: 10, y: 50, label: 'Data Ingestion', status: 'success' },
    { id: 'transform', x: 35, y: 50, label: 'Transform', status: 'processing' },
  ...
  const getNodeMetrics = (nodeId: string) => {
    const metrics: Record<string, { throughput: string; latency: string; records: string }> = {
      ingest: { throughput: '12.5 MB/s', latency: '45ms', records: '2.4M' },
      transform: { throughput: '8.2 MB/s', latency: '120ms', records: '1.8M' },
  ...
  return (
    <div className="relative w-full h-[300px] sm:h-[350px] bg-gradient-to-b from-background to-muted/30 rounded-2xl border border-border overflow-hidden">
      {/* Background Grid */}
      ...
      {hoveredNode && (
        <div
          ...
          style={{
            left: hoveredNode === 'ingest' ? '5%' : hoveredNode === 'load' ? '75%' : '35%',
            top: '60%',
          }}
        >
          <div className="flex items-center gap-2 mb-2">
            <div className={`w-2 h-2 rounded-full ${getStatusDot(nodes.find((n) => n.id === hoveredNode)?.status || 'idle')}`} />
            <span className="font-semibold text-sm">{nodes.find((n) => n.id === hoveredNode)?.label}</span>
          </div>
          <div className="space-y-1 text-xs text-muted-foreground">
            <div className="flex justify-between gap-4">
              <span>Throughput:</span>
              <span className="text-foreground font-medium">{getNodeMetrics(hoveredNode).throughput}</span>
            </div>
            <div className="flex justify-between gap-4">
              <span>Latency:</span>
              <span className="text-foreground font-medium">{getNodeMetrics(hoveredNode).latency}</span>
            </div>
            <div className="flex justify-between gap-4">
              <span>Records:</span>
              <span className="text-foreground font-medium">{getNodeMetrics(hoveredNode).records}</span>
            </div>
          </div>
        </div>
      )}
